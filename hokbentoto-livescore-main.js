...
(function(config){
  ...
  function boot(){
    ...
    function renderTicker(){
      ...
      var up=matches.filter(function(m){return m.state==='upcoming';}).slice(0,5);
      var texts=[];
      live.forEach(function(m){texts.push('● LIVE '+m.home+' '+scoreText(m)+' '+m.away+(m.minute?' ('+m.minute+')':''));});
      up.forEach(function(m){texts.push((m.kickoff_wib||'--:--')+' '+m.home+' vs '+m.away);});
      ticker.textContent=texts.length?texts.join('   ◆   '):'Tidak ada pertandingan hari ini saat ini';
  ...
  function sendHeightLoop(){
    var lastH=0;
    function measure(){var el=document.getElementById('hbtls-1');return el?Math.ceil(el.getBoundingClientRect().height):0;}
    function send(force){
      try{
        var h=measure();
