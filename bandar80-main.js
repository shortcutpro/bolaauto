...
/* ═══════════════════════════════════════════════
   ELEMEN ROOT
═══════════════════════════════════════════════ */
var root = document.getElementById('bandar80-root');

// Inject progress bar CSS
(function(){
  var s=document.createElement('style');
  s.textContent=
    '#bandar80-root .ibc-progress-wrap{width:80%;max-width:320px;margin-top:2px;}'
    +'#bandar80-root .ibc-progress-track{width:100%;height:10px;background:rgba(255,255,255,0.06);border-radius:8px;overflow:hidden;border:1px solid #29A8E026;position:relative;}'
    +'#bandar80-root .ibc-progress-fill{height:100%;width:0%;background:linear-gradient(90deg,#145a7a,#29A8E0,#6cc9f0);border-radius:8px;transition:width .4s ease;box-shadow:0 0 12px #29A8E066,inset 0 1px 0 rgba(255,255,255,0.25);}'
    +'#bandar80-root .ibc-progress-pct{color:#29A8E0;font-size:12px;font-weight:700;letter-spacing:1px;text-align:center;margin-top:6px;text-shadow:0 0 8px #29A8E060;}';
...
async function fetchHTML(url){
  ...
  for(var i=0;i<totalProxies;i++){
    ...
    try{
      ...
      console.log('[BANDAR80] Mencoba '+px.name+'…');
      ...
      console.log('[BANDAR80] OK via '+px.name);
      updateProgress(60);
      return html;
    }catch(e){
      fetchDone = true;
      console.warn('[BANDAR80] '+px.name+' gagal:',e.message);
    }
  }
...
function buildOutputHTML(leagues){
  ...
  return '<!-- Generated by BANDAR80 Auto Embed -->\n'
+'<style>\n'
...
+'#bandar80-root{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("'+BG_IMAGE+'") center/cover fixed no-repeat #000;padding:1px 0;border-radius:0;min-height:100px;}\n'
...
async function loadAndRender(){
  try {
    showLoading('Memuat Prediksi BANDAR80…', 0);
    ...
    console.error('[BANDAR80 Embed]',e);
