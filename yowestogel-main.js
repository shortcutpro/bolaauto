...
async function fetchHTML(url){
  for(var i=0;i<PROXIES.length;i++){
    var px=PROXIES[i];
    try{
      showLoading('Memuat Prediksi YOWES TOGEL…','['+( i+1)+'/'+PROXIES.length+'] Mencoba '+px.name+'…');
...
async function loadAndRender(){
  try {
    showLoading('Memuat Prediksi YOWES TOGEL…','Mengambil data dari sumber…');
    var html = await fetchHTML(SOURCE_URL);
    showLoading('Memuat Prediksi YOWES TOGEL…','Memproses data prediksi…');
    ...
    showLoading('Memuat Prediksi YOWES TOGEL…','Membangun tampilan…');
