...
var SITE_NAME     = 'YOWES TOGEL';
var SITE_LOGO     = 'https://yowestogel70.com/assets/img/akz/logo.png?v=666';
var MBAPPE_GIF    = 'https://photoku.io/images/2026/05/31/giffmbappee-finall.gif';
var MARQUEE_TEXT  = '👑 PREDIKSI BOLA TERUPDATE ! Tunggu apa lagi? Daftar di YOWES TOGEL dan nikmati pengalaman taruhan terbaik! 👑';
var COLOR_MAIN    = '#FF9C00';
var COLOR_TEXT    = '#ffe9c8';
var BG_IMAGE      = 'https://cdn.areabermain.club/assets/cdn/az7/2026/01/14/20260114/6abc2835164afa38d3e769afa1e8d4d4/background-yowestogel.jpg';
var BG_OVERLAY    = 'rgba(28,4,0,0.55), rgba(42,8,0,0.62)';
...
/* ═══════════════════════════════════════════════
   ELEMEN ROOT
═══════════════════════════════════════════════ */
var root = document.getElementById('yowestogel-root');

// Inject progress bar CSS
(function(){
  var s=document.createElement('style');
  s.textContent=
    '#yowestogel-root .ibc-progress-wrap{width:80%;max-width:320px;margin-top:2px;}'
    +'#yowestogel-root .ibc-progress-track{width:100%;height:10px;background:rgba(255,255,255,0.06);border-radius:8px;overflow:hidden;border:1px solid #FF9C0026;position:relative;}'
    +'#yowestogel-root .ibc-progress-fill{height:100%;width:0%;background:linear-gradient(90deg,#8c5500,#FF9C00,#ffb94c);border-radius:8px;transition:width .4s ease;box-shadow:0 0 12px #FF9C0066,inset 0 1px 0 rgba(255,255,255,0.25);}'
    +'#yowestogel-root .ibc-progress-pct{color:#FF9C00;font-size:12px;font-weight:700;letter-spacing:1px;text-align:center;margin-top:6px;text-shadow:0 0 8px #FF9C0060;}';
...
function showLoading(msg, pct){
  ...
  root.innerHTML =
    ...
    '<img src="https://yowestogel70.com/assets/img/akz/logo.png?v=666" alt="YOWES TOGEL" style="width:110px;max-height:72px;object-fit:contain;filter:drop-shadow(0 0 16px #FF9C00);margin-bottom:8px;"/>'+
    '<div class="ibc-spinner"></div>'+
    '<div class="ibc-loading-text">⚽ '+(msg||'Memuat Prediksi YOWES TOGEL…')+'</div>'+
    (hasBar?
...
async function fetchHTML(url){
  ...
  for(var i=0;i<totalProxies;i++){
    ...
    try{
      ...
      console.log('[YOWES TOGEL] Mencoba '+px.name+'…');
      ...
      console.log('[YOWES TOGEL] OK via '+px.name);
      updateProgress(60);
      return html;
    }catch(e){
      fetchDone = true;
      console.warn('[YOWES TOGEL] '+px.name+' gagal:',e.message);
    }
  }
...
function parseRawHtml(html){
  ...
  buttons.forEach(function(btn){
    ...
    if(txt.indexOf('PREDIKSI')<0) return;
  ...
  targetBodies.forEach(function(item){
    ...
    lines.forEach(function(line){
      ...
      var isNoise=/https?:\/\/|www\.|@/i.test(clean)||/^\d+$/.test(clean)||clean.length>100||/klik|login|daftar|register|promo|bonus|copyright/i.test(clean);
...
function parseGeneric(doc){
  ...
  var NOISE=/klik|disini|whatsapp|telegram|instagram|copyright|privacy|https?:|www\.|@/i;
  var result=[];var capturing=false;var lastLg=false;
  text.split(/\r?\n/).map(function(l){ return preNorm(l); }).filter(function(l){ return l.length>1; }).forEach(function(line){
    var isMatch=MATCH_RE.test(line);
    if(!capturing&&isMatch) capturing=true;
    if(!capturing) return;
    if(isMatch){ result.push(line); lastLg=false; return; }
    var isNoise=NOISE.test(line)||/^\d+$/.test(line)||line.length>90;
...
function buildDateRange(leagues){
  ...
  if(first.m===last.m) return first.d+' - '+last.d+' '+BULAN[first.m]+' '+yr;
...
function buildOutputHTML(leagues){
  ...
  return '<!-- Generated by YOWES TOGEL Auto Embed -->\n'
+'<style>\n'
...
async function loadAndRender(){
  try {
    showLoading('Memuat Prediksi YOWES TOGEL…', 0);
    ...
    console.error('[YOWES TOGEL Embed]',e);
