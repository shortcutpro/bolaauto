...
console.log("[bolaauto mancingduit] date-fix-2026-08-24 aktif");
(function(){
...
var SITE_NAME     = 'MANCING DUIT';
var SITE_LOGO     = 'https://cdn.areabermain.club/assets/cdn/az5/2026/09/16/20260916/710b5546b38d761c24f131f706426431/logo.png';
var MBAPPE_GIF    = 'https://photoku.io/images/2026/05/31/giffmbappee-finall.gif';
var MARQUEE_TEXT  = '🐟 PREDIKSI BOLA TERUPDATE ! Tunggu apa lagi? Daftar di MANCING DUIT dan nikmati pengalaman taruhan terbaik! 🐟';
var COLOR_MAIN    = '#29B6E8';
var COLOR_TEXT    = '#e3f5ff';
var BG_IMAGE      = 'https://cdn.areabermain.club/assets/cdn/az9/2026/08/28/20260828/d1f5e158a8737de103b042bd222a8d45/mancingduit-background-28juli2026-1.jpg';
var BG_OVERLAY    = 'rgba(4,20,52,0.55), rgba(6,30,74,0.62)';
var AUTO_REFRESH  = 5 * 60 * 1000; // 5 menit
var CACHE_KEY     = 'mancingduit_pred_cache_v1';
...
/* ═══════════════════════════════════════════════
   ELEMEN ROOT
═══════════════════════════════════════════════ */
var root = document.getElementById('mancingduit-root');

// Inject progress bar CSS
(function(){
  var s=document.createElement('style');
  s.textContent=
    '#mancingduit-root .ibc-progress-wrap{width:80%;max-width:320px;margin-top:2px;}'
    +'#mancingduit-root .ibc-progress-track{width:100%;height:10px;background:rgba(255,255,255,0.06);border-radius:8px;overflow:hidden;border:1px solid #FFC80026;position:relative;}'
    +'#mancingduit-root .ibc-progress-fill{height:100%;width:0%;background:linear-gradient(90deg,#8c6e00,#FFC800,#ffd84c);border-radius:8px;transition:width .4s ease;box-shadow:0 0 12px #FFC80066,inset 0 1px 0 rgba(255,255,255,0.25);}'
    +'#mancingduit-root .ibc-progress-pct{color:#FFC800;font-size:12px;font-weight:700;letter-spacing:1px;text-align:center;margin-top:6px;text-shadow:0 0 8px #FFC80060;}';
...
function showLoading(msg, pct){
  ...
  root.innerHTML =
    ...
    '<img src="https://cdn.areabermain.club/assets/cdn/az5/2026/09/16/20260916/710b5546b38d761c24f131f706426431/logo.png" alt="MANCING DUIT" style="width:110px;max-height:72px;object-fit:contain;filter:drop-shadow(0 0 16px #FFC800);margin-bottom:8px;"/>'+
    '<div class="ibc-spinner"></div>'+
    '<div class="ibc-loading-text">⚽ '+(msg||'Memuat Prediksi MANCING DUIT…')+'</div>'+
    (hasBar?
...
async function fetchHTML(url){
  ...
  for(var i=0;i<totalProxies;i++){
    ...
    try{
      ...
      console.log('[MANCING DUIT] Mencoba '+px.name+'…');
      ...
      console.log('[MANCING DUIT] OK via '+px.name);
      updateProgress(60);
      return html;
    }catch(e){
      fetchDone = true;
      console.warn('[MANCING DUIT] '+px.name+' gagal:',e.message);
    }
  }
...
function buildOutputHTML(leagues){
  ...
  return '<!-- Generated by MANCING DUIT Auto Embed -->\n'
+'<style>\n'
...
+'#mancingduit-root{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("'+BG_IMAGE+'") center/cover fixed no-repeat #000;padding:1px 0;border-radius:0;min-height:100px;}\n'
...
async function loadAndRender(){
  ...
  if(cache && cache.html){
    ...
    try {
      ...
      writeCache(freshBg);
      injectHTML(freshBg);
    } catch(e){ console.warn('[MANCING DUIT] refresh latar gagal:', e.message); }
    return;
  }

  // Belum ada cache (kunjungan pertama) — baru tampilkan loading
  try {
    showLoading('Memuat Prediksi MANCING DUIT…', 0);
    var output = await fetchFreshOutput(true);
    writeCache(output);
    injectHTML(output);
  } catch(e) {
    console.error('[MANCING DUIT Embed]',e);
...
async function backgroundRefresh(){
  try {
    ...
    writeCache(fresh);
    injectHTML(fresh);
  } catch(e){ console.warn('[MANCING DUIT] auto-refresh gagal:', e.message); }
}
