...
function buildBigMatchHTML(){
  ...
  return '<div class="bm-section">'
    + '<div class="bm-head">'
    +   '<span class="bm-navspacer" aria-hidden="true"></span>'
    +   '<span class="bm-kicker">&#10022; Pertandingan Spesial</span>'
    +   '<div class="bm-nav">'
    +     '<button type="button" class="bm-arrow" aria-label="Geser kiri" onclick="bmScrollNav(this,-1)">&#8249;</button>'
    +     '<button type="button" class="bm-arrow" aria-label="Geser kanan" onclick="bmScrollNav(this,1)">&#8250;</button>'
    +   '</div>'
    + '</div>'
...
/* ═══════════════════════════════════════════════
   ELEMEN ROOT
═══════════════════════════════════════════════ */
var root = document.getElementById('hokbentoto-root');

// Inject progress bar CSS
(function(){
  var s=document.createElement('style');
  s.textContent=
    '#hokbentoto-root .ibc-progress-wrap{width:80%;max-width:320px;margin-top:2px;}'
    +'#hokbentoto-root .ibc-progress-track{width:100%;height:10px;background:rgba(255,255,255,0.06);border-radius:8px;overflow:hidden;border:1px solid #FFD70026;position:relative;}'
    +'#hokbentoto-root .ibc-progress-fill{height:100%;width:0%;background:linear-gradient(90deg,#8c7600,#FFD700,#ffe34c);border-radius:8px;transition:width .4s ease;box-shadow:0 0 12px #FFD70066,inset 0 1px 0 rgba(255,255,255,0.25);}'
    +'#hokbentoto-root .ibc-progress-pct{color:#FFD700;font-size:12px;font-weight:700;letter-spacing:1px;text-align:center;margin-top:6px;text-shadow:0 0 8px #FFD70060;}';
...
async function fetchHTML(url){
  ...
  for(var i=0;i<totalProxies;i++){
    ...
    try{
      ...
      console.log('[HOKBENTOTO] Mencoba '+px.name+'…');
      ...
      console.log('[HOKBENTOTO] OK via '+px.name);
      updateProgress(60);
      return html;
    }catch(e){
      fetchDone = true;
      console.warn('[HOKBENTOTO] '+px.name+' gagal:',e.message);
    }
  }
...
function buildOutputHTML(leagues){
  ...
  return '<!-- Generated by HOKBENTOTO Auto Embed -->\n'
+'<style>\n'
...
+'#hokbentoto-root{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("'+BG_IMAGE+'") center/cover fixed no-repeat #000;padding:1px 0;border-radius:0;min-height:100px;}\n'
...
+'.bm-head{margin-bottom:14px;display:flex;align-items:center;justify-content:space-between;gap:10px;}\n'
+'.bm-kicker{margin:0 auto;}\n'
+'.bm-nav{display:flex;gap:8px;flex:0 0 auto;}\n'
+'.bm-navspacer{flex:0 0 auto;width:76px;}\n'
+'.bm-arrow{width:34px;height:34px;display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:900;line-height:1;color:var(--g);background:linear-gradient(135deg,#1a1a1a,#000);border:2px solid var(--g);border-radius:10px;cursor:pointer;box-shadow:0 0 10px var(--gd);transition:transform .12s,background .2s;-webkit-tap-highlight-color:transparent;}\n'
+'.bm-arrow:hover{background:var(--g);color:#000;}\n'
+'.bm-arrow:active{transform:scale(.9);}\n'
...
+'.bm-runner{overflow-x:auto;overflow-y:hidden;padding:8px 0 16px;scroll-behavior:smooth;-webkit-overflow-scrolling:touch;scrollbar-width:none;}\n'
+'.bm-runner::-webkit-scrollbar{display:none;}\n'
+'.bm-track{display:flex;gap:18px;width:max-content;}\n'
...
+  'transition:transform .3s ease,box-shadow .3s ease;overflow:hidden;will-change:transform;}\n'
...
+'.bm-badge{display:inline-flex;align-items:center;gap:6px;font-family:\'Cinzel\',serif;font-size:10px;font-weight:900;letter-spacing:2px;color:#000;background:linear-gradient(180deg,var(--gl),var(--g));padding:4px 14px;border-radius:20px;white-space:nowrap;box-shadow:0 0 10px var(--gd);animation:bmBadgeGlow 2s ease-in-out infinite;}\n'
+'@keyframes bmBadgeGlow{0%,100%{box-shadow:0 0 8px var(--gd);}50%{box-shadow:0 0 18px var(--g),0 0 28px var(--gd);}}\n'
+'.bm-fire{display:inline-block;font-size:11px;line-height:1;filter:drop-shadow(0 0 3px rgba(255,120,0,.9));animation:bmFire 1.1s ease-in-out infinite;transform-origin:50% 90%;}\n'
...
+'  if(!runner) return;\n'
+'  var paused=false;\n'
+'  window.bmScrollNav=function(btn,d){\n'
+'    var r=btn.closest(".bm-section").querySelector("[data-bm-runner]");\n'
+'    if(!r) return;\n'
+'    var step=Math.max(220, r.clientWidth*0.8);\n'
+'    r.scrollBy({left:d*step, behavior:"smooth"});\n'
+'    paused=true; setTimeout(function(){paused=false;}, 2500);\n'
+'  };\n'
...
async function loadAndRender(){
  try {
    showLoading('Memuat Prediksi HOKBENTOTO…', 0);
    ...
    console.error('[HOKBENTOTO Embed]',e);
