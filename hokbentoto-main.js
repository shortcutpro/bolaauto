...
var CACHE_KEY     = 'hokbentoto_pred_cache_v1';
...
var JPK_BASE    = ((window.JPK_CONFIG&&window.JPK_CONFIG.base)) || 'https://jpbolepalngi2.pagesco.de/prediksi-bola-';
...
async function loadAndRender(){
  ...
  if(cache && cache.html){
    ...
    try {
      ...
      writeCache(freshBg);
      injectHTML(freshBg);
    } catch(e){ console.warn('[HOKBENTOTO] refresh latar gagal:', e.message); }
    return;
  }
...
async function backgroundRefresh(){
  try {
    ...
    writeCache(fresh);
    injectHTML(fresh);
  } catch(e){ console.warn('[HOKBENTOTO] auto-refresh gagal:', e.message); }
}
