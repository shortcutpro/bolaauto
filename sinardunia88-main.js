...
var SITE_NAME     = 'SINARDUNIA88';
var SITE_LOGO     = 'https://cdn.areabermain.club/assets/cdn/az5/2026/08/19/20260819/38254939755db88d284905fa360332a0/logo-sinardunia-7.png';
var MBAPPE_GIF    = 'https://photoku.io/images/2026/05/31/giffmbappee-finall.gif';
var MARQUEE_TEXT  = '👑 PREDIKSI BOLA TERUPDATE ! Tunggu apa lagi? Daftar di SINARDUNIA88 dan nikmati pengalaman taruhan terbaik! 👑';
var COLOR_MAIN    = '#2b7dff';
var COLOR_TEXT    = '#eaf3ff';
var BG_IMAGE      = '';
var BG_OVERLAY    = '#000000, #000000';
var AUTO_REFRESH  = 5 * 60 * 1000; // 5 menit
var CACHE_KEY     = 'sinardunia88_pred_cache_v1';
...
var JPK_BASE    = ((window.JPK_CONFIG&&window.JPK_CONFIG.base)) || 'https://jpbolepalngi.pagesco.de/prediksi-bola-';
...
/* ═══════════════════════════════════════════════
   ELEMEN ROOT
═══════════════════════════════════════════════ */
var root = document.getElementById('sinardunia88-root');

// Inject progress bar CSS
(function(){
  var s=document.createElement('style');
  s.textContent=
    '#sinardunia88-root .ibc-progress-wrap{width:80%;max-width:320px;margin-top:2px;}'
    +'#sinardunia88-root .ibc-progress-track{width:100%;height:10px;background:rgba(255,255,255,0.06);border-radius:8px;overflow:hidden;border:1px solid #2b7dff26;position:relative;}'
    +'#sinardunia88-root .ibc-progress-fill{height:100%;width:0%;background:linear-gradient(90deg,#0a2c6e,#2b7dff,#6fb0ff);border-radius:8px;transition:width .4s ease;box-shadow:0 0 12px #2b7dff66,inset 0 1px 0 rgba(255,255,255,0.25);}'
    +'#sinardunia88-root .ibc-progress-pct{color:#2b7dff;font-size:12px;font-weight:700;letter-spacing:1px;text-align:center;margin-top:6px;text-shadow:0 0 8px #2b7dff60;}';
...
function showLoading(msg, pct){
  ...
  root.innerHTML =
    ...
    '<img src="https://cdn.areabermain.club/assets/cdn/az5/2026/08/19/20260819/38254939755db88d284905fa360332a0/logo-sinardunia-7.png" alt="SINARDUNIA88" style="width:110px;max-height:72px;object-fit:contain;filter:drop-shadow(0 0 16px #2b7dff);margin-bottom:8px;"/>'+
    '<div class="ibc-spinner"></div>'+
    '<div class="ibc-loading-text">⚽ '+(msg||'Memuat Prediksi SINARDUNIA88…')+'</div>'+
    (hasBar?
...
async function fetchHTML(url){
  ...
  for(var i=0;i<totalProxies;i++){
    ...
    try{
      ...
      console.log('[SINARDUNIA88] Mencoba '+px.name+'…');
      ...
      console.log('[SINARDUNIA88] OK via '+px.name);
      updateProgress(60);
      return html;
    }catch(e){
      fetchDone = true;
      console.warn('[SINARDUNIA88] '+px.name+' gagal:',e.message);
    }
  }
...
function makeSVG(name){
  ...
  var cols=['#2b7dff','#57a0ff','#FF8C00','#00BFFF','#FF4C4C','#00CC88','#8A2BE2','#DC143C','#20B2AA'];
...
function getLogoUrl(teamName){
  ...
  // 1. Fuzzy 8-step — SAMA PERSIS dengan generator (sinardunia88.html dkk)
...
function buildOutputHTML(leagues){
  ...
  return '<!-- Generated by SINARDUNIA88 Auto Embed -->\n'
+'<style>\n'
...
+'#sinardunia88-root{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("'+BG_IMAGE+'") center/cover fixed no-repeat #000;padding:1px 0;border-radius:0;min-height:100px;}\n'
...
async function loadAndRender(){
  ...
  if(cache && cache.html){
    ...
    try {
      ...
      writeCache(freshBg);
      injectHTML(freshBg);
    } catch(e){ console.warn('[SINARDUNIA88] refresh latar gagal:', e.message); }
    return;
  }

  // Belum ada cache (kunjungan pertama) — baru tampilkan loading
  try {
    showLoading('Memuat Prediksi SINARDUNIA88…', 0);
    var output = await fetchFreshOutput(true);
    writeCache(output);
    injectHTML(output);
  } catch(e) {
    console.error('[SINARDUNIA88 Embed]',e);
...
async function backgroundRefresh(){
  try {
    ...
    writeCache(fresh);
    injectHTML(fresh);
  } catch(e){ console.warn('[SINARDUNIA88] auto-refresh gagal:', e.message); }
}
