...
var CACHE_KEY     = 'ziatogel_pred_cache_v1';
...
async function loadAndRender(){
  ...
  if(cache && cache.html){
    ...
    try {
      ...
      writeCache(freshBg);
      injectHTML(freshBg);
    } catch(e){ console.warn('[ZIA TOGEL] refresh latar gagal:', e.message); }
    return;
  }
...
async function backgroundRefresh(){
  try {
    ...
    writeCache(fresh);
    injectHTML(fresh);
  } catch(e){ console.warn('[ZIA TOGEL] auto-refresh gagal:', e.message); }
}
