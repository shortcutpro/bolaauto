...
console.log("[bolaauto ruangwd] date-fix-2026-08-24 aktif");
(function(){
...
var SITE_NAME     = 'RUANG WD';
var SITE_LOGO     = 'https://cdn.areabermain.club/assets/cdn/az5/2026/09/17/20260917/ba5b6c700dc14f65946009082c308130/logo-1.png';
var MBAPPE_GIF    = 'https://photoku.io/images/2026/05/31/giffmbappee-finall.gif';
var MARQUEE_TEXT  = '🐉 PREDIKSI BOLA TERUPDATE ! Tunggu apa lagi? Daftar di RUANG WD dan nikmati pengalaman taruhan terbaik! 🐉';
...
/* ═══════════════════════════════════════════════
   ELEMEN ROOT
═══════════════════════════════════════════════ */
var root = document.getElementById('ruangwd-root');
...
async function fetchHTML(url){
  ...
  for(var i=0;i<totalProxies;i++){
    ...
    try{
      ...
      console.log('[RUANG WD] Mencoba '+px.name+'…');
      ...
      console.log('[RUANG WD] OK via '+px.name);
      updateProgress(60);
      return html;
    }catch(e){
      fetchDone = true;
      console.warn('[RUANG WD] '+px.name+' gagal:',e.message);
    }
  }
...
function buildOutputHTML(leagues){
  ...
  return '<!-- Generated by RUANG WD Auto Embed -->\n'
+'<style>\n'
...
+'#ruangwd-root{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("'+BG_IMAGE+'") center/cover fixed no-repeat #000;padding:1px 0;border-radius:0;min-height:100px;}\n'
...
async function loadAndRender(){
  ...
  if(cache && cache.html){
    ...
    try {
      ...
      writeCache(freshBg);
      injectHTML(freshBg);
    } catch(e){ console.warn('[RUANG WD] refresh latar gagal:', e.message); }
    return;
  }

  // Belum ada cache (kunjungan pertama) — baru tampilkan loading
  try {
    showLoading('Memuat Prediksi RUANG WD…', 0);
    var output = await fetchFreshOutput(true);
    writeCache(output);
    injectHTML(output);
  } catch(e) {
    console.error('[RUANG WD Embed]',e);
...
async function backgroundRefresh(){
  try {
    ...
    writeCache(fresh);
    injectHTML(fresh);
  } catch(e){ console.warn('[RUANG WD] auto-refresh gagal:', e.message); }
}
