...
var CACHE_KEY     = 'linetogel_pred_cache_v1';
...
async function loadAndRender(){
  ...
  if(cache && cache.html){
    ...
    try {
      ...
      writeCache(freshBg);
      injectHTML(freshBg);
    } catch(e){ console.warn('[LINE TOGEL] refresh latar gagal:', e.message); }
    return;
  }
...
async function backgroundRefresh(){
  try {
    ...
    writeCache(fresh);
    injectHTML(fresh);
  } catch(e){ console.warn('[LINE TOGEL] auto-refresh gagal:', e.message); }
}
